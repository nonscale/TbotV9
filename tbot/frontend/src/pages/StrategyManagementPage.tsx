...
import { Title, Table, Button, Group, ActionIcon, Modal, TextInput, Textarea, Switch, Stack } from '@mantine/core';
...
// --- Type definitions ---
interface ConditionNode { type: 'condition'; value: string; }
...
const EMPTY_STRATEGY_FORM: StrategyFormValues = {
    ...
    first_scan: {
        type: 'group',
        operator: 'AND',
        children: [{ type: 'condition' as const, value: 'amount > 100000000' }],
    },
};
...
export default function StrategyManagementPage() {
  ...
  const handleEditOpen = (strategy: Strategy) => {
    ...
    form.setValues({
      ...
      first_scan: strategy.scan_rules.first_scan || EMPTY_STRATEGY_FORM.first_scan,
    });
    openModal();
  };
  ...
  return (
    <>
      <Modal opened={modalOpened} onClose={closeModal} title={editingStrategy ? 'Edit Strategy' : 'Add New Strategy'} size="xl">
        <form onSubmit={form.onSubmit(handleSubmit)}>
          <Stack>
            ...
            <Title order={4} mt="md">1st Scan Rules</Title>
      ...
      <Table>
        <Table.Thead><Table.Tr><Table.Th>Name</Table.Th><Table.Th>Schedule</Table.Th><Table.Th>Status</Table.Th><Table.Th>Actions</Table.Th></Table.Tr></Table.Thead>

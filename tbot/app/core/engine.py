...
from tbot.app.core.logic_parser import logic_parser, LogicParserError # Import the parser
...
async def _perform_scan(strategy: Strategy):
    ...
    broker = UpbitBroker()
    first_scan_rule = strategy.scan_rules.get("first_scan", "")
    ...
    while True:
        try:
            ...
            # Apply the dynamic 1st phase scan rule
            try:
                filtered_df = logic_parser.apply(tickers_df, first_scan_rule)
    ...
    # Send final status update when the loop exits
    await manager.broadcast("scan_status_update", {
        "strategy_id": strategy.id,
        "status": "STOPPED",
...
def stop_scan(strategy_id: int):
    ...
    if strategy_id in active_scans:
        # The task will clean itself up upon cancellation
        pass
